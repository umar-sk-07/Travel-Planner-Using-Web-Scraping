...
/**
 * Scrapes hotels from Yatra using an already-connected Puppeteer page.
 * The page is provided by the worker (connected via Bright Data).
 */
export const startHotelScraping = async (
  ...
  return page.evaluate((): Hotel[] => {
    ...
    for (const card of cardCandidates) {
      ...
      if (
        !/\boff\b/i.test(text) &&
        !/\bhotel\b/i.test(text) &&
        !/\bresort\b/i.test(text)
      ) {
        continue;
      }
      ...
      const title =
        (
          (
            card.querySelector(
              "h1, h2, h3, h4, .hotel-name, [class*='hotel']"
            ) as HTMLElement | null
          )?.innerText || ""
        )
          .replace(/\s+/g, " ")
